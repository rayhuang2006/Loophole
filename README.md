...
**大小。** 然後把任何「刪掉也不改變破壞了什麼」的語句刪掉，直到刪不動為止。
剩下的就是仍然破壞同樣東西的最小程式。

第二步這一條測試取代了一整疊特例。`widen` 到原本的位寬、`revive` 一個活著的人、
沒有人讀的定義、掛在整數 exploit 旁邊卻跟它無關的 `define everyone := {}`——
全部同一條規則處理掉，因為它們都不是承重牆。

我一開始是分三條規則各補一次的，寫完發現那是補丁堆疊：
每加一種操作就要想它有沒有新的「無效」形式。改成最小見證之後，
那三條規則連同它們的輔助函式一起刪掉了，程式反而短了七十行。

修完之後：帶人的世界 38 種變 **9 種**，沒有人的世界回到**正好 6 種**，
跟加 Phase 2 之前逐字相同。在一個沒有人的世界裡，別名軸產生不出任何新機制，
而搜尋器自己認出了這件事。
